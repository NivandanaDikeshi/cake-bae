...
<<<<<<< HEAD
  ...
  // on every Role CRUD action was the root cause of the "forced logout on
  // CRUD" bug — see the onAuthStateChanged effect for details.
...
<<<<<<< HEAD
  ...
  // meant Firebase's auth listener was torn down and re-subscribed every
  // single time `roles` changed reference — which happens on every Role
  // CRUD action (addRole/updateRole/deleteRole all call setRoles()), and
  // even on the initial roles fetch. Re-subscribing forces onAuthStateChanged
  // to fire again immediately with whatever Firebase's auth state is at that
  // instant; if that came back momentarily null/delayed, the `else` branch
  // below would wipe currentUser, which the layout treats as "logged out"
  // and redirects to /admin/login. This is what caused CRUD actions and
  // navigation to randomly force a re-login.
  //
  // The fix: subscribe ONCE (empty dependency array) and read roles via
